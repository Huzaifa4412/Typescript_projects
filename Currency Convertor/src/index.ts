...
do {
  ...
  let ask_user = await inquirer.prompt([
    {
      message: chalk.yellow.visible.bold(
        "Choose your currency that you want to convert from"
      ),
      name: "from",
      type: "list",
    ...
    {
      message: chalk.yellow.visible.bold(
        "Choose your currency that you want to convert in"
      ),
      name: "to",
      type: "list",
      choices: ["USD", "PKR", "INR", "GBP", "EUR", "IRR", "SAR", "JPY", "CAD"],
  ...
  console.log(
    chalk.yellow.visible.bold(`Your amount is ${chalk.blue.bold(result.toFixed(2))}`)
  );
